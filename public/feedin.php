    ...
    // move uploaded file to upload directory
     ...
     "../uploads/{$_FILES['json'] ['name']}")  ){
       ...
       foreach ($data as $key=>$datum) {
         // calculate percentage upload to mysql
         $percent = $key/count($data)*100;
         $source = $datum["source"];
         $target = $datum["target"];
         $weight = 100;
         if ($datum["type"] == "inhibit"){
           $type = "inhibit";
         }
         else {
           $type = "excite";
         ...
         query("INSERT INTO `inibyt`.`userlinks` (`source`, `target`, `type`,
                                                 `weight`, `linkid`, `userid`)
         VALUES (?, ?, ?, ?, NULL, ?)",$source, $target, $type, $weight,
                                                       $_SESSION["id"]);
                                                     }
